...
1. 在目标 VRChat Worlds 项目中导入资源包，将 `Prefabs/Core/BH2VSQ_BASE_Core.prefab` 拖入场景。
2. 将 `Teleport/Point_*` 移到建筑中的目的地，调整其 `AreaTrigger` 碰撞体。
3. 在场景实例的 `Authentication/TOTPAuthManager` 设置成员与管理员 Base32 密钥。不要把真实密钥提交到公共仓库；上传的世界依然向客户端暴露密钥。
4. 将世界出生点放在公共安全位置，检查只有一个点标记 `isSafeFallback`。
5. 选中核心实例，运行 **BH2VSQ BASE → 验证配置**，修正错误。
6. 用 ClientSim 和至少两个 VRChat 客户端测试游客、成员、管理员、迟到加入、传送同意、楼层状态及广播队列。

## 重新生成

构建会覆盖生成的预制体与示例场景。特定世界的点位、触发器及密钥应保存在独立场景实例中。修改 `Data/Default/DefaultLocationDatabase.asset` 只影响后续生成的初始点。已有场景实例可直接增删 `TeleportPoint`，无须修改代码。交互式构建前请保存已修改场景。